...
import com.welkinwits.adapter.SingleExamListingAdapter
import com.welkinwits.databinding.FragmentSingleExamBinding
import com.welkinwits.model.QuestionModelData
import com.welkinwits.service.request.exams.submitAction.ExamSubmitRequest
import com.welkinwits.service.request.exams.submitAction.SubmitDatum
...
class SingleExamFragment : BaseFragment(R.layout.fragment_single_exam) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.singleExamSubmitAnswerResponse.observe(viewLifecycleOwner) {
            Log.d(TAG, "singleExamSubmitAnswerResponse: Success"+it?.toString())
            it?.let {
                Handler(Looper.getMainLooper()).postDelayed({
                    Toast.makeText(context,""+it.data?.data?.message,Toast.LENGTH_SHORT).show()
    ...
    private fun configureRecyclerView() {
        mAdapter = SingleExamListingAdapter(requireContext()) // Use requireContext() instead of activity?.applicationContext
    ...
    override fun onResume() {
        super.onResume()
        viewModel.clearSingleExamSubmitAnswerResponse()
        mExamId?.toInt()?.let { viewModel.getSingleExam(it) }
